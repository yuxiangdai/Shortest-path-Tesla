...
def filter_keys(lat, lon, keys):
    global sts, sta_info
    res_keys = []
    for key in keys:
        if dist(lat, lon, sta_info[sts[key]]['latitude'], sta_info[sts[key]]['longitude']) <= 480:
            res_keys.append(key)
    return res_keys
...
def get_sort_keys_by_spherical_dist_plus(lat, lon, keys):
    
    global sts, sta_info
    for i in range(len(keys)):
        for j in range(i+1, len(keys)):
            t_dist_i = dist(sta_info[sts[keys[i]]]['latitude'], sta_info[sts[keys[i]]]['longitude'], lat, lon)
            t_dist_j = dist(sta_info[sts[keys[j]]]['latitude'], sta_info[sts[keys[j]]]['longitude'], lat, lon)
            if t_dist_i > t_dist_j:
                keys[i], keys[j] = keys[j], keys[i]
    return keys
...
def get_top_k(lat, lon, lat_g, lon_g, k):
    global sts, sta_info
    keys = get_sort_keys_by_spherical_dist_plus(lat_g, lon_g, filter_keys(lat, lon, get_keys_plus()))
    res = []
    for i in range(len(keys)):
        res.append(keys[i])
    return res
...
def get_closer(st1, st2, st3):
    if get_dist(st1, st3) > get_dist(st2, st3):
        return True
    else:
        return False
...
def SPFA():
    ...
    cnt = 3000
    while q.empty() != True and cnt > 0:
        cnt = cnt - 1
        curr = q.get()
        print("cnt:"+str(cnt))
        print(shortest_time[curr])
        print(total_dist[curr])
        in_queue[curr] = False
        keys = get_top_k(sta_info[sts[curr]]['latitude'], sta_info[sts[curr]]['longitude'], sta_info[sts[end_pt]]['latitude'], sta_info[sts[end_pt]]['longitude'], 5)
        
        print(keys)
        for i in keys:
            if i != curr and get_dist(sts[curr], sts[i]) <= 480 and get_closer(sts[curr], sts[i], sts[len(sts)-1]) == True:
